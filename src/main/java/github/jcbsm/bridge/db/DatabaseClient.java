package github.jcbsm.bridge.db;

import github.jcbsm.bridge.config.BridgeConfig;
import github.jcbsm.bridge.db.exceptions.NoResultException;
...
public class DatabaseClient {
    ...
    private static final DatabaseClient client = new DatabaseClient();
    private final Logger logger = LoggerFactory.getLogger(DatabaseClient.class.getName());

    private static final Path DB_FILE_PATH = BridgeConfig.getConfigDir();
    ...
    private Connection connect() throws SQLException {
        String connString = String.format("jdbc:sqlite:%s\\linked-accounts.db", BridgeConfig.getConfigDir().toString().replace(".\\", ""));
    ...
    public UUID getCurrentUUID(User user) throws Exception {
        ...
        try (Connection conn = this.connect()){
            ...
            if (stmt.execute()){
                ...
                throw new NoResultException("member_uuid", String.valueOf(user.getIdLong()));
            }
        }
    ...
    public String getCurrentUsername(User user) throws Exception{
        ...
        try (Connection conn = this.connect()){
            ...
            if (stmt.execute()){
                ...
                throw new NoResultException("member_name", String.valueOf(user.getIdLong()));
            }
        }
